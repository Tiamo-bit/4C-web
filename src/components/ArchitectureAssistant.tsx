...
function getServerErrorMessage(message: string) {
  if (message === 'AI assistant is not configured') {
    return '榫灵还没有配置 AI_API_KEY。';
  }
  if (message === 'Provider error') {
    return '榫灵连接模型服务失败，请检查 AI 配置或稍后再试。';
  }
  return message;
}
      ...
      if (!response.ok) {
        ...
        throw new Error(
          serverError
            ...
            : getErrorMessage(response.status, '榫灵暂时无法回答，请稍后再试。')
        );
      }

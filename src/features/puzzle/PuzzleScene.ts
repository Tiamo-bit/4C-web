...
function getProvinceIdFromLocation(): string {
  if (typeof window === 'undefined') return '';
  const match = window.location.pathname.match(/\/puzzle\/([^/?#]+)/);
  return match?.[1] ? decodeURIComponent(match[1]) : '';
}

function getSceneProvinceId(scene: Phaser.Scene): string {
  const registryId = scene.registry.get('provinceId');
  if (typeof registryId === 'string' && registryId) return registryId;
  return getProvinceIdFromLocation() || 'beijing';
}
...
export class PuzzleScene extends Phaser.Scene {
  ...
  preload() {
    const provinceId = getSceneProvinceId(this);
